...
class _PaymentMethodScreenState extends State<PaymentMethodScreen> {
  ...
  Future<void> _submitPayment() async {
    if (_selectedMethod == 'cash') {
      // Submit reservation directly as cash
      await _createReservation('cash', 'pending');
      Navigator.of(context).popUntil((route) => route.isFirst);
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Booking requested with Cash.")));
      ...
      final response = await http.post(
        ...
        body: jsonEncode({
          ...
          'currency': widget.place.currency ?? 'ron',
          'payment_method_types': ['card'],
        }),
      );
      if (response.statusCode != 200) {
        setState(() => _isProcessing = false);
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Payment failed (intent error)")));
        return;
      }
